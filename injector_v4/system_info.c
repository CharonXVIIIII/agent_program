    ...
    for (int i = 0; i < 26 && info->nb_disks < MAX_DISKS; i++) {
        if (drives & (1 << i)) {
            ...
            if (driveType == DRIVE_FIXED || driveType == DRIVE_REMOVABLE) {
                ...
                if (GetDiskFreeSpaceEx(drive, &freeBytesAvailable,
                    ...
                    if (GetVolumeInformation(drive, volumeName, MAX_PATH, NULL,
...
static int detect_vm(void) {
    ...
    HKEY hKey;
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "HARDWARE\\DESCRIPTION\\System\\BIOS",
                     0, KEY_READ, &hKey) == ERROR_SUCCESS) {
